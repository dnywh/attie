...
const API_BASE_URL = 'https://site.api.espn.com/apis/site/v2/sports/rugby-league/3/scoreboard';
...
export async function GET(request, { params }) {
    ...
    console.log(`[NRL API] Getting ${direction} games for NRL`);
    console.log(`[NRL API] Date range: ${dateFrom} to ${dateTo}`);
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 429) {
                console.log(`[NRL API] Rate limit exceeded for NRL`);
                return NextResponse.json(
            ...
            if (response.status === 204) {
                console.log(`[NRL API] No content returned for NRL`);
                return NextResponse.json({ matches: [], message: 'No matches found' });
            }

            console.log(`[NRL API] Error ${response.status} for NRL`);
            return NextResponse.json(
        ...
        console.log(`[NRL API] Found ${matchCount} matches for NRL`);
        ...
        console.error('[NRL API] Fetch error:', error);
        return NextResponse.json(
